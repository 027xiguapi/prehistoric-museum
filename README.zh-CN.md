...
同样的 18 种动物，还一起站进首页内嵌的可走动 3D 动物园里；也可以单独打开全屏页 **`/{locale}/zoo/`**。

- **一切都是真实大小。** 每个展品模型都会被缩放到「它自己的体型数据」所描述的那一档，所以 12 米的霸王龙站在 1.75 米的小探险家旁边，真的就是 12 米。已发布的模型本身大小各不相同，这里没有一个数字是靠肉眼估的。
- **是一个有地貌的山谷，不是一份名单。** 草原和森林在温暖的南边，一道可以走上去的断崖把翼龙托在空中，冰川和潟湖在寒冷的北边——和展台页面用的是同一套分区和同一套说法。
- **自己走，或者被带着走。** W A S D 或方向键行走，按住 Shift 快跑；点一下地面，小探险家会绕开围栏和水面走过去；点一下无人机，就让自动导览带你逛完整座园子。
- **走过去，就算认识了。** 走近它，它会跟你打招呼、报上名字，并播放它自己那段已经审过的旁白。想再听一次，点它就好。
- **探险手册。** 认识的每一位朋友都会记在里面，点任意一条就能直接走回那个观看点；集齐 18 位会撒下彩带。
- **不会重复下载。** 首页内嵌的园区要等你滚动到它、或者自己点开，才开始加载；开始运行后，一离开屏幕就会停止绘制。它的全屏按钮可以就地铺满整屏，关闭按钮则把 WebGL 上下文和已经请进来的模型立刻还给设备。展品模型也只在你周围陆续请进来，走远之后又会放回去，所以在平板上也只用掉很小一块内存。

「鱼龙类」和「蛇颈龙类」这两件展品代表的是类群而不是某一个物种。它们的模型按已公布范围的中位数来定大小，博物馆原本的那句提醒也一直跟着那个数字，和展台页面完全一致。

整座动物园里没有任何动物叫声。没有人知道剑龙当年怎么叫，所以你听到的每个声音，都是脚步声、提示音，或者博物馆自己的旁白。
