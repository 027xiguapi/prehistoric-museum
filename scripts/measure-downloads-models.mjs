/* global window */
// Measures the TRUE world-space bounds of every converted model-downloads
// model with three.js (GLTFLoader dequantizes KHR_mesh_quantization into node
// transforms, which plain gltf-transform walks cannot see on skinned meshes).
// Same measurement strategy as scripts/measure-1048-models.mjs.
//
// Output: converted-downloads/downloads-measurements.json — per slug min/max/size.
// Usage: node scripts/measure-downloads-models.mjs [--only=slug1,slug2]
...
for (const entry of activeDownloads()) {
  ...
  currentModelPath = join(convertedRoot, entry.slug, 'model.glb')
  ...
  try {
    ...
    measurements.push({ slug: entry.slug, status: `failed: ${String(error).slice(0, 200)}` })
    console.log(`✗ ${entry.slug}: ${String(error).slice(0, 160)}`)
    continue
  }
  ...
  console.log(
    `✓ ${entry.slug}  ${size.map((value) => value.toFixed(1)).join(' × ')}  longest=${Math.max(...size).toFixed(1)} (${['x', 'y', 'z'][size.indexOf(Math.max(...size))]})`,
  )
}

await browser.close()
await writeFile(
  join(convertedRoot, 'downloads-measurements.json'),
  JSON.stringify(measurements, null, 2),
)
console.log(`\n${measurements.filter((row) => row.status === 'ok').length}/${measurements.length} measured → converted-downloads/downloads-measurements.json`)
