...
// every model-downloads draft, so the per-animal baked yaw (head → +Z) can be
// decided by inspection. Tiger is included as a control: it is known to face
// +Z at yaw 0. Mirrors scripts/render-1048-contact-sheets.mjs.
//
// Input:  src/content/animals/<slug>/model/model.glb (normalized pass-1 draft)
// Output: converted-downloads/<slug>/orientation.png (4 panels side by side).
// Usage: node scripts/render-downloads-contact-sheets.mjs [--only=slug1,slug2]
...
const SLUGS = [
  ...
  ...activeDownloads().map((entry) => entry.slug),
]
...
const pitchFlag = process.argv.find((argument) => argument.startsWith('--pitch='))

const PANEL = 280
const YAWS = yawsFlag
  ? yawsFlag.slice(7).split(',').map(Number)
  : [0, 90, 180, 270]
const PITCH = pitchFlag ? Number(pitchFlag.slice(8)) : 10
...
for (const slug of SLUGS) {
  ...
  for (const yaw of YAWS) {
    await page.goto(
      `https://render.test/?w=${PANEL}&h=${PANEL}&yaw=${yaw}&pitch=${PITCH}`,
    )
    try {
  ...
  const sheet = await sharp({
    ...
    .png()
    .toBuffer()
  const outDir = slug === 'tiger' ? outputRoot : join(outputRoot, slug)
  await mkdir(outDir, { recursive: true })
  const outPath = slug === 'tiger'
    ? join(outDir, `tiger-${OUT_NAME}-control.png`)
    : join(outDir, `${OUT_NAME}.png`)
