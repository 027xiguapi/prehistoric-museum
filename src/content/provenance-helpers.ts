...
export const reviewedBackgroundSources = {
  ...
  maiasaura: {
    landscape: {
      ...
      prompt:
        '为中文亲子WonZoo生成一张纯环境背景图。晚白垩世北美河漫滩与疏林，温暖自然、半写实高品质自然史绘本风格，金棕色草地、低矮蕨类、远处针叶林与轻薄云层，柔和清晨散射光。横向 16:9 构图；明确把地平线抬高到画面顶部向下约 43% 的位置；画面中央约 60% 宽度必须是一整片连续、平整、可供居中 3D 陆生动物落脚的地面，脚下区域不要有沟、石块或高草，透视与地面接触关系清晰；重要景物只放在两侧和远景。不要出现任何动物、人物、骨骼、文字、标志、展柜、UI 或水印。背景本身，不要画边框。',
      bytes: 2_975_870,
      sha256:
    ...
    portrait: {
      ...
      generatedOn: '2026-07-30',
      prompt:
        '为中文亲子WonZoo生成一张纯环境背景图。晚白垩世北美河漫滩与疏林，温暖自然、半写实高品质自然史绘本风格，金棕色草地、低矮蕨类、远处针叶林与轻薄云层，柔和清晨散射光。竖向 9:16 构图；明确把远处林线和地平线放在画面顶部向下约 52% 的位置；画面中央约 78% 宽度从地平线到底部必须是一整片连续、平整、可供居中 3D 陆生动物落脚的地面，脚下区域不要有沟、石块或高草，透视与地面接触关系清晰；高树和重要景物只放在两侧边缘和远景，不遮挡中央。不要出现任何动物、人物、骨骼、文字、标志、展柜、UI 或水印。背景本身，不要画边框。',
      bytes: 2_711_111,
      sha256:
  ...
  megalodon: {
    landscape: {
      ...
      generatedOn: '2026-07-30',
      prompt:
        '为中文亲子WonZoo生成一张纯环境背景图。新近纪开放大陆架海域，广阔而沉静的深蓝海水，半写实高品质自然史绘本风格；水面与明亮波纹仅占画面顶部约 16%，柔和斜射光束制造巨大尺度感，远方水体渐变为较深的蓝色，沙质海床和低矮岩脊只在底部约 82% 以下隐约出现。横向 16:9 构图；画面中央约 70% 宽度和 60% 高度必须是开阔、低对比、无遮挡的游动空间，适合居中叠加一只巨型史前鲨鱼 3D 模型；两侧可有极少量远处水下地形作为尺度线索，但不可以有鱼群或其他动物。气氛壮阔但不恐怖，不要血腥。不要出现任何动物、鱼、人物、潜水员、船、骨骼、牙齿、文字、标志、展柜、UI 或水印。背景本身，不要画边框。',
      bytes: 1_784_569,
      sha256:
  ...
  plesiosaurus: {
    landscape: {
      ...
      generatedOn: '2026-07-30',
      prompt:
        '为中文亲子WonZoo生成一张纯环境背景图。侏罗纪温暖浅海与外海交界，半写实高品质自然史绘本风格，清澈蓝绿色海水，水面光带位于画面顶部约 15%，微弱阳光束，下方约 78% 处才出现低矮沙质海床与少量远处海草、菊石壳和岩礁作为时代线索。横向 16:9 构图；画面中央约 65% 宽度和 55% 高度必须保持开阔、安静、低对比的可游动水域，适合居中叠加一只长颈海生爬行动物的 3D 模型；不要让海草、岩石、气泡群或光束切过动物轮廓。色彩柔和但有层次，环境可信、亲子友好。不要出现任何动物、鱼、人物、潜水员、船、骨骼、文字、标志、展柜、UI 或水印。背景本身，不要画边框。',
      bytes: 1_540_514,
      sha256:
    ...
    portrait: {
      ...
      generatedOn: '2026-07-30',
      prompt:
        '为中文亲子WonZoo生成一张纯环境背景图。侏罗纪温暖浅海与外海交界，半写实高品质自然史绘本风格，清澈蓝绿色海水，水面光带位于画面顶部约 12%，微弱阳光束，沙质海床只在画面底部约 82% 以下出现，边缘有少量远处海草、菊石壳与低矮岩礁作为时代线索。竖向 9:16 构图；画面中央约 78% 宽度从顶部 18% 到底部 78% 必须保持开阔、安静、低对比的可游动水域，适合居中叠加一只长颈海生爬行动物的 3D 模型；不要让海草、岩石、气泡群或强光束切过动物轮廓。色彩柔和但有层次，环境可信、亲子友好。不要出现任何动物、鱼、人物、潜水员、船、骨骼、文字、标志、展柜、UI 或水印。背景本身，不要画边框。',
      bytes: 1_727_512,
      sha256:
